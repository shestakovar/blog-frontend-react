...
import AuthService, { SuccessLogin } from '../services/AuthService';
...
const RegisterPage: FC = () => {
  ...
  const [userData, setUserData] = useState<IPrintField>({
    username: '',
    password: '',
    first_name: '',
    last_name: '',
  ...
  const [register, isLoading, error, clearError, validated] = useFormFetching(async () => {
    ...
    const thunk = (response: SuccessLogin) => (history.push(`/users/${response.userid}`));
